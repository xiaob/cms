...
public class UpdatePictureUtils {
	 ...
	 * 获得按尺寸放大或缩小后的图片
	 */
	public BufferedImage createThumbnail(String img, int num)
			throws FileNotFoundException, IOException {
		BufferedImage img1 = ImageIO.read(new File(img));
		BufferedImage thumbnail = Scalr.resize(img1, Scalr.Method.SPEED,
				Scalr.Mode.AUTOMATIC, num, Scalr.OP_ANTIALIAS);
		return thumbnail;
	}
	 ...
	 * 按照指定大小切割图片并存到服务器
	 */
	public void updateArticlePicture(long id, String image, String picture)
			throws FileNotFoundException, IOException {
		...
		finalImg = this.cut(descImg, (descWidth - width) / 2,
				(descHeight - height) / 2, width, height);
		...
		g.dispose();
		// 输出为文件
		ImageIO.write(tag, "JPEG", new File(webroot + "/upload/photo/" + id
				+ "_big.jpg"));
		...
		g1.dispose();
		// 输出为文件
		ImageIO.write(tag1, "JPEG", new File(webroot + "/upload/photo/" + id
				+ "_small.jpg"));
	}

	public void updatePicture(long id, String image, String picture)
			throws FileNotFoundException, IOException {
		BufferedImage bi = ImageIO.read(new File(image));
		int srcWidth = bi.getWidth(); // 源图宽度
		int srcHeight = bi.getHeight(); // 源图高度
		...
		g.dispose();
		// 输出为文件
		ImageIO.write(tag, "JPEG", new File(webroot + "/upload/" + id
				+ "_picture.jpg"));
	}
